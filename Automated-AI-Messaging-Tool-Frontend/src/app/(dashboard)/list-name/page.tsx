...
const FileUploadPage = () => {
  ...
  const onDropRejected = useCallback((rejectedFiles: any[]) => {
    ...
    if (file) {
      const validTypes = ['.csv', '.xlsx', '.xls'];
      const fileExtension = file.file.name.toLowerCase().substring(file.file.name.lastIndexOf('.'));
      
      if (!validTypes.includes(fileExtension)) {
        setError(`❌ Invalid file type! Please select only CSV or Excel files (.csv, .xlsx, .xls). You selected: ${file.file.name}`);
      } else if (file.file.size > 100 * 1024 * 1024) {
        setError('❌ File too large! Please select a file smaller than 100MB.');
      } else {
        setError('❌ File not supported. Please select a valid CSV or Excel file.');
      }
    }
  }, []);
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        const response = await fetch('http://103.215.159.51:8001/api/upload-from-frontend?userId=cmc1ralor0000s159apuq8j90', {
          method: 'POST',
          body: formData
        });
        ...
        if (response.ok) {
          ...
          setUploadResult({
            id: result.fileUploadId || result.id,
            filename: selectedFile.name,
            totalWebsites: result.totalWebsites || result.fileUpload?.totalWebsites || 0,
            processedWebsites: 0,
            failedWebsites: 0,
            totalChunks: result.totalChunks || result.fileUpload?.totalChunks || 1,
            completedChunks: 0,
            status: result.status || result.fileUpload?.status || 'PROCESSING',
            createdAt: new Date().toISOString(),
            websites: [],
            processingInfo: result.processingInfo || { estimatedTime: '2-5 minutes', status: 'Processing started' }
          });
          setSelectedFile(null);

          // Start tracking progress for large files
          if ((result.totalChunks || result.fileUpload?.totalChunks || 1) > 1) {
            setIsTrackingProgress(true);
            startProgressTracking(result.fileUploadId || result.id);
          }
        } else {
    ...
    const trackProgress = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Update uploadResult with latest data
          setUploadResult(prev => prev ? {
            ...prev,
            processedWebsites: data.fileUpload.processedWebsites || 0,
            failedWebsites: data.fileUpload.failedWebsites || 0,
            completedChunks: data.fileUpload.completedChunks || 0,
            status: data.fileUpload.status
          } : prev);
                ...
                {uploadResult.processingInfo && (
                  <Alert severity="info" icon={<InfoIcon />}>
                    <Typography variant="body2">
                      <strong>Processing Info:</strong> {uploadResult.processingInfo.status}
                      <br />
                      <strong>Estimated Time:</strong> {uploadResult.processingInfo.estimatedTime}
                    </Typography>
                  </Alert>
                )}
