...
function getColorStyle({ variant, theme, color, type }: AvatarStyleProps) {
  ...
  // Handle missing color properties in MUI v7
  const lighter = colors.lighter || alpha(main, 0.12);
  const light = colors.light || alpha(main, 0.3);
...
const AvatarStyle = styled(MuiAvatar, { shouldForwardProp: (prop: string) => prop !== 'color' && prop !== 'type' && prop !== 'size' })<StyleProps>(
  ({ theme, color, type, size }: { theme: any; color: ColorProps; type?: AvatarTypeProps; size?: SizeProps }) => ({
    ...getSizeStyle(size),
