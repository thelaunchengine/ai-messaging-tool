...
app = FastAPI(title="Simple Working AI Generator", version="1.0.0")

@app.get("/health")
async def health_check():
    return {"status": "healthy", "message": "Simple Working AI Generator is running"}

@app.post("/api/ai/generate-real")
async def generate_real_ai_messages(website_data: List[Dict[str, Any]]):
    """Generate real AI messages using Gemini API - Simple working version"""
    try:
        if not website_data:
